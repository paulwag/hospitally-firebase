<template>
  <v-container>
    <v-toolbar
      ...
      fluid
    >
      <v-toolbar-title>Offene Stellen</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-icon
...
import VacancyList from '@/components/volunteer/VacancyList'
export default {
  components: {
    SearchComponent,
    FilterComponent,
          ...
          doc.ref
            ...
            .then((vacancyResponse) => {
              for (const vacancy of vacancyResponse.docs) {
                ...
                vacancyObj.adress = doc.data().address
                vacancyObj.hospitalID = doc.id
